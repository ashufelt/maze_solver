...
class Maze():
    ...
    def _solve_r(self, i, j):
        ...
        if i == self.__num_cols-1 and j == self.__num_rows-1:
            return True
        if self._cells[i][j].has_right_wall is False and i < self.__num_cols-1:
            if self._cells[i+1][j].visited is False:
                self._cells[i][j].draw_move(self._cells[i+1][j])
                next_cell_result = self._solve_r(i+1, j)
                if next_cell_result is True:
                    return True
                self._cells[i][j].draw_move(self._cells[i+1][j], undo=True)
        if self._cells[i][j].has_bottom_wall is False and j < self.__num_rows-1:
            if self._cells[i][j+1].visited is False:
                self._cells[i][j].draw_move(self._cells[i][j+1])
                next_cell_result = self._solve_r(i, j+1)
                if next_cell_result is True:
                    return True
                self._cells[i][j].draw_move(self._cells[i][j+1], undo=True)
        if self._cells[i][j].has_left_wall is False and i > 0:
            if self._cells[i-1][j].visited is False:
                self._cells[i][j].draw_move(self._cells[i-1][j])
                next_cell_result = self._solve_r(i-1, j)
                if next_cell_result is True:
                    return True
                self._cells[i][j].draw_move(self._cells[i-1][j], undo=True)
        if self._cells[i][j].has_top_wall is False and j > 0:
            if self._cells[i][j-1].visited is False:
                self._cells[i][j].draw_move(self._cells[i][j-1])
                next_cell_result = self._solve_r(i, j-1)
                if next_cell_result is True:
